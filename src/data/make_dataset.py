...
def read_data_from_files(files):
    ...
    # Define time columns (verify these match your actual column names!)
    TIME_COLUMNS = ["epoch (ms)", "time (01:00)", "elapsed (s)"]

    # Set index using correct column name (ONE SPACE!)
    acc_df.index = pd.to_datetime(acc_df["epoch (ms)"], unit="ms")
    gyr_df.index = pd.to_datetime(gyr_df["epoch (ms)"], unit="ms")

    # Drop time columns safely
    acc_df = acc_df.drop(columns=TIME_COLUMNS, errors="ignore")
    gyr_df = gyr_df.drop(columns=TIME_COLUMNS, errors="ignore")
